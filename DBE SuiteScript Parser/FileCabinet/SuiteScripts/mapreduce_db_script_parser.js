...
define([
	...
	function(
		...
		function _getInput() {
			...
			var param = script.getParameter({name: 'custscript_scripts_to_parse'});
			...
			parseType = scriptCurr.getParameter({ name: 'custscript_parse_by' });
			//parseType = parseInt(parseType);
			parseRecords = scriptCurr.getParameter({ name: 'custscript_scripts_to_parse' });
			fileIds = scriptCurr.getParameter({ name: 'custscript_files_to_parse' });
			//create master record
			//customrecord_main_parse_summary
			var custRec = record.create({type: 'customrecord_main_parse_summary',  isDynamic: true});
				...
				case 'record':
					if(parseRecords){
						...
						if(results[0]){
							...
							custRec.setValue({	fieldId: 'custrecord_master_record_type',
								value: recType	});
							...
							for (var l = 0; l < results.length; l++){
								var scriptid = results[l].getValue({name: 'internalid', join: 'script'});
								if(scriptid){

									result.push({	id:scriptid,
										type: 'script',
										masterId: masterId,
										parseType:parseType	,
				...
				case 'internalid':
					if(parseRecords){
						...
						for (var j = 0; j < splitScriptIds.length; j++){
							result.push({	id:splitScriptIds[j],
								type: 'script',
								masterId: masterId,
								parseType:parseType
							});
					...
					if(fileIds){
						...
						for (var k = 0; k < splitFileIds.length; k++){
							result.push({id:splitFileIds[k], type: 'file', masterId: masterId});
						}
					}
					break;
		...
		function _reduce(context) {
			...
			try{
				...
				if(id && id.type == 'script'){
					result = search.lookupFields({columns: ['scriptfile', 'apiversion','defaultfunction', 'name', 'scripttype'], type: 'script', id: id.id});
					//scriptName
					...
					result = search.lookupFields({columns: ['name', 'scripttype'], type: 'file', id: id.id});
				...
				var custRec = record.create({type: 'customrecord_parse_details',  isDynamic: true});
				custRec.setValue({fieldId:'custrecord_parse_script_name', value: result.name });
				custRec.setValue({fieldId:'custrecord_parse_script_version', value: result.apiversion });
				if(result.scripttype)
					custRec.setValue({fieldId:'custrecord_parse_script_type', value: result.scripttype[0].text });
				custRec.setValue({fieldId:'custrecord_parse_script_id', value: id.id });
				custRec.setValue({fieldId:'custrecord_parse_file_name', value: scriptFile });
				custRec.setValue({fieldId:'custrecord_parse_info', value: convertToHTML(parsedObj) });
				custRec.setValue({fieldId:'custrecord_parse_grading', value: summary.initGrade });
				if(summary.summary)
					custRec.setValue({fieldId:'custrecord_parse_summary', value: summary.summary });
				else
					custRec.setValue({fieldId:'custrecord_parse_summary', value: 'No heavy operations found' });
				if(summary.usage)
					custRec.setValue({fieldId:'custrecord_parse_governance_use', value: summary.usage });
				else
					custRec.setValue({fieldId:'custrecord_parse_governance_use', value: 'No significant governance usage found'});
				custRec.setValue({fieldId:'custrecord_total_usage_count', value: summary.totalUsage});
				custRec.setValue({fieldId:'custrecord_summary_num_lines', value:lines });
				custRec.setValue({fieldId:'custrecord_parent_master_summary', value: masterId});
				custRec.setValue({fieldId:'custrecord_parse_api_efficiency', value: summary.recommendation});
				custRec.setValue({fieldId:'custrecord_total_operations_found', value: summary.apiCount });
				custRec.setValue({fieldId:'custrecord_total_rec_count', value: summary.recCount });
				id = custRec.save();

				context.write({key: id, value: summary});
			}
			catch(e){
				...
				if(masterId){
					context.write({key: masterId, value: err});
				}
		...
		function _extractScript(fileName, apiversion, main, id){
			...
			var fileId = _getFileIdFromName({name: fileName});
			try{
			...
			var syntax = esprima.parseModule(fileContents.content, {loc:true});
			...
			if(apiversion == '1.0' || apiversion == 1.0){
				...
				if(main){ //the main 1.0 script file
					...
					var rec = record.load({type: 'script', id: id});
					...
					var defaultFunction = rec.getValue({fieldId:'defaultfunction'});
					if(defaultFunction){
						...
						//user event
						var beforeLoad = rec.getValue({fieldId:'beforeloadfunction'});
						var beforeSubmit = rec.getValue({fieldId:'beforesubmitfunction'});
						var afterSubmit = rec.getValue({fieldId:'aftersubmitfunction'});

						//client side
						var pageInit = rec.getValue({fieldId: 'pageinitfunction'});
						var saveRecord = rec.getValue({fieldId: 'saverecordfunction'});
						var validateField = rec.getValue({fieldId: 'validatefieldfunction'});
						var fieldChanged = rec.getValue({fieldId: 'fieldchangedfunction'});
						var postSourcing = rec.getValue({fieldId: 'postsourcingfunction'});
						var lineInit = rec.getValue({fieldId: 'lineinitfunction'});
						var validateLine = rec.getValue({fieldId: 'validatelinefunction'});
						var validateInsert = rec.getValue({fieldId: 'validateinsertfunction'});
						var validateDelete = rec.getValue({fieldId: 'validatedeletefunction'});
						var recalc = rec.getValue({fieldId: 'recalcfunction'});
					...
					for(var q = 0; q < rec.getLineCount({ sublistId: 'libraries'}); q++){
		...
		function buildTree(obj, functionDetails, loop, modules, condition){
				...
				obj.type == NAMES.CALL_EXPRESSION)
			{
				...
				if(obj.type == NAMES.ASSIGNMENT_EXPRESSION){
					//functionDetails
					settingGlobalVar = isSettingGlobalVar({functionDetails:functionDetails, op:obj});
				}
			...
			for(var i = 0; i < operations.length; i++){
				...
				if(	val &&
					...
					)
					{
						...
						return {type: val.type, nsmod: val.nsmod};
		...
		function extractFunctions(main, func, parsedObj, index, isEntry){  //pass script
			...
			for(var y = 0; y < main.functionList.length; y++){
				...
				if(operations.length > 0){
					...
					parsedObj.functions.push({'filename':main.name, 'fname':functionDetails.functionName, 'initiator':func.initiator, 'index': index, 'linecount':lineloc, loop:func.loop });
					for(var x = 0; x < operations.length; x++){
						...
						if(operation.isNetSuiteAPI == true){
							...
							parsedObj.functions.push({'operation':operation, index: index});
						}
						else{
							...
							if(!isModule){
								...
								if(	moduleCall &&
									...
									var result = chkRecSave({current: moduleCall, operations: functionDetails});
									...
									if(result){
										...
										parsedObj.functions.push({'operation':moduleCall, index: index});
									}
		...
		function getNestedObject(obj, arr){
			...
			if(obj.hasOwnProperty("object")){
				...
				arr.push({object: obj.object, property: obj.property});
		...
		function getSummary(parsedObj){
				...
				if(val){
					...
					if(op && op.method && op.isNetSuiteAPI == true){
						...
						if(method.indexOf('nlapi') > -1 && (method == 'nlapiLoadRecord' || method == 'nlapiSubmitField' || method == 'nlapiLookupField' ||
							method == 'nlapiRequestURL' || method == 'nlapiSubmitRecord')){
							if(opList.length == 0)
								opList.push({method: method, loop: loop, saveFound: saveFound, line:line, arr:[{type:type, count:1}]});
							else{
								var exists = false;
								for(var o = 0; o < opList.length; o++){
									if(nsmod == opList[o].module && method == opList[o].method){
										var count = countRecTypes({arr: opList[o].arr, type: type});
							...
							if(opList.length == 0)
								opList.push({module: nsmod, method: method, loop: loop, saveFound: saveFound, line:line, arr:[{type:type, count:1}]});
							else{
								var exists = false;
								for(var o = 0; o < opList.length; o++){
									if(nsmod == opList[o].module && method == opList[o].method){
			...
			for(var j = 0; j < opList.length; j++){
				...
				var result = printTotalCount({arr:opList[j].arr});
				if(method.indexOf('nlapi') > -1 && (method == 'nlapiLoadRecord' || method == 'nlapiSubmitField' || method == 'nlapiLookupField' ||
					method == 'nlapiRequestURL' || method == 'nlapiSubmitRecord')){
					if(	result.htmlStr &&
						(
		...
		function listToHTML(opList){
			...
			for(var j = 0; j < opList.length; j++){
				...
				var method = opList[j].method;
				var typeArr = opList[j].arr;
				var result = printTotalCount({arr:typeArr});
				if(method.indexOf('nlapi') > -1 && (method == 'nlapiLoadRecord' || method == 'nlapiSubmitField' || method == 'nlapiLookupField' ||
					method == 'nlapiSubmitRecord')){
		...
		function printTotalCount(obj){
			...
			htmlStr += '\n';
			return {htmlStr:htmlStr, totalCount: totalCount};
		}
		...
		function _summarize(summary) {
				...
				if(value)
				{
					...
					if(value.indexOf('Error:') > -1)
					{
						...
						if(arr1.length == 0){
							...
							getMasterSummary({arr1: arr1, arr2: arr2});
						}
						//end operations list
			...
			try{
				if(keyId){
					...
					var masterId = search.lookupFields({type: 'customrecord_parse_details', id: keyId, columns: 'custrecord_parent_master_summary'});
					...
					if(masterId && masterId["custrecord_parent_master_summary"]){
						...
						var rec = record.load({type: 'customrecord_main_parse_summary', id: id});
						log.debug('master summary',  summary);
						rec.setValue({fieldId: 'custrecord_parse_ave_script_grade', value: getAveGrade(grade)});
						rec.setValue({fieldId: 'custrecord_parsing_summary', value: summary});
						...
						rec.setValue({fieldId: 'custrecord_parse_sum_line_count', value: lineText});
						rec.setValue({fieldId: 'custrecord_record_sum_operations_found', value: totalAPICount});
						rec.setValue({fieldId: 'custrecord_record_sum_gov_count', value: totalUsageCount});
						rec.setValue({fieldId: 'custrecord_record_sum_gov_usage', value: totalUsage});
						rec.setValue({fieldId: 'custrecord_record_sum_total_rec_count', value: totalRecommendationCount});
						rec.setValue({fieldId: 'custrecord_record_sum_efficiency', value: totalAPI});
						rec.save();
					}
					else{
						var rec = record.load({type: 'customrecord_main_parse_summary', id: keyId});
						rec.setValue({fieldId: 'custrecord_parse_errors', value: errors});
						rec.save();
					}
				}
			}
		...
		function getMasterSummary(obj){
			...
			for (var v = 0; v < arr2.length; v++){
				...
				for (var x = 0; x < arr1.length; x++){
					...
					if(arr1Mod == arr2Mod && arr1Method == arr2Method){
						for(b in arr2Arr){
							...
							if(!recFound){
								arr1Arr.push({type:type2, count:arr2Arr[b].count });
							}
						}
						found = true;
					}
				...
				if(!found){
					arr1.push({module:arr2Mod, method:arr2Method, arr: arr2[v].arr});
				}
			}
